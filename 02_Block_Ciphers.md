...
$F: K x X -> Y$ è una funzione se esiste un algoritmo efficiente che calcola $F(k, x)$ per ogni $k \in K$ e $x \in X$.
**PRP(Pseudo-Random Permutation):**
$E: K x X -> X$ è una permutazione se esiste un algoritmo deterministico efficiente che calcola $E(k, x)$, $E(k, .)$ è 1 a 1 per ogni $k \in K$ e esiste un algoritmo efficiente che invertendo $E(k, x)$ calcola $D(k, y)$ per ogni $k \in K$ e $y \in X$.
...
$F: K x X -> Y$ è una PRF.
$Funs[X, Y] = {f: X -> Y}$ è l'insieme delle funzioni da X a Y, di grandezza $|Funs[X, Y]| = |Y|^{|X|}$(ogni elemento di X ha |Y| possibili valori).
...
Una PRF è sicura se una funzione randomica di Fun[X, Y] è indistinguibile da una funzione randomica di S_f.

*Definizione:*
Consideriamo una PRF F: K x X -> Y. Per b = 0, 1 definiamo l'esperimento EXP(b) come:
...
*Definizione:*
Consideriamo una PRP E: K x X -> X. Per b = 0, 1 definiamo l'esperimento EXP(b) come:
...
**Feistel Network:**
$\forall (arbitrarie) f_1, f_2, ..., f_d: {0, 1}^n -> {0, 1}^n$ non per forza invertibili, Feistel Netwrok $F: {0, 1}^{2n} -> {0, 1}^{2n}$ è invertibile.
...
- 16 round di Feistel Network: in ogni round si utilizza una sottochiave di 48-bit che si combina a metà dell'output del round precedente, si applica una funzione non invertibile e si scambia i due blocchi
...
Allora $\forall m, c: \exists al massimo 1 k: c = DES(k, m)$ con probabilità $>= 1 - \frac{1}{256}\approx 99.5%$.
...
- 3DES: 
    - $3E: K^3 x M -> M \rightarrow 3E(k_1, k_2, k_3, m) = E(k_1, D(k_2, E(k_3, m)))$
...
- DESX:
    - $EX: K x M -> M \rightarrow EX(k_1, k_2, k_3, m) = k_1 \oplus E(k_2, m \oplus k_3)$
    - lunghezza chiave: 64 + 56(k_2) + 64 = 184 bits
    - attacco semplice in tempo $\approx 2^{64 + 56} = 2^{120}$

*Perchè non si utilizza il 2DES?*
$2E: K^2 x M -> M \rightarrow 2E(k_1, k_2, m) = E(k_1, E(k_2, m))$
...
Step 2:
- per ogni $k_i \in {0, 1}^{56}$ guarda se $D(k_i, c)$ è nella seconda colonna della tabella(quindi guarda ne decifrando il messaggio con ogni chiave trova una corrispondenza tra il messaggio cifrato nella tabella e il messaggio che ha decifrato, se la trova ha le due chiavi)
...
- è un cifrario a blocchi che funziona iterativamente:
    - block size: 128-bit
    - key size: $128(3.4*10^{38} chiavi possibili), 192(6.2*10^{57} chiavi possibili), 256(1.1*10^{77} chiavi possibili)$ bits
    - 10, 12, 14 round
